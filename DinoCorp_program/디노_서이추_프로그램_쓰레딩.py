...
class secondThread(QThread):
    ...
    def run(self):
        ...
        if self.manage == 1:
            self.manage_neighbor(self.input_id)
...
# 이웃수, 공감, 댓글 싸그리
            ...
            for keyword in input_keyword:
                ...
                while avg < cnt:
                    ...
                    try:
                        ...
                        # 이웃 추가 버튼 클릭
                        time.sleep(2.5)
                        self.driver.find_element(By.CSS_SELECTOR, "#postlist_block > div.post_block__Q6T_o > div > div > button:nth-child(3)").send_keys(Keys.ENTER)
    ...
    def manage_neighbor(self, input_id):
        ...
        try:
            ...
            self.driver.find_element(By.CSS_SELECTOR, "#buddylist_config_anchor").click()

            print("내가 추가한 이웃 클릭")

            # 이웃그룹 선택하기 위해 iframe 접속
            self.driver.switch_to.frame("papermain")
            self.driver.find_element(By.CSS_SELECTOR, "#wrap > ul > li._nclk\(bas_neimgr\.gnei\) > a").click()
            print("이웃그룹 클릭")

            time.sleep(2)

            # 이웃수 리스트
            neighbor_num_list = self.driver.find_elements(By.CSS_SELECTOR, ".num")[ : : 2]
            ...
            # 받은 신청이 없는 경우
            try:
                self.driver.find_element(By.CSS_SELECTOR, ".none")
                self.update.emit("받은 신청이 없습니다.")
                ...
                for i in range(len(hrefs) + 1):
                    ...
                    while 1:
                        ...
                        # 전체선택 -> 수락
                        self.driver.find_element(By.CSS_SELECTOR, "#inviteMe > div:nth-child(7) > div > span.all_select > input").click()
                        print("전체선택 클릭")
                        
                        self.driver.find_element(By.CSS_SELECTOR, "#inviteMe > div:nth-child(7) > div > span:nth-child(2) > button").click()
                        print("수락 클릭")
                        ...
                        # 그룹 선택
                        groups = self.driver.find_elements(By.CSS_SELECTOR, "#groupIdSelector > option")
                        groups[neighbor_index].click()

                        # 확인
                        self.driver.find_element(By.CSS_SELECTOR, "#footer > input[type=image]").click()
                        time.sleep(3)

                        is_err = self.driver.find_elements(By.CSS_SELECTOR, ".ngb_err")
                        print(len(is_err))
            ...
            self.update.emit("이웃신청 받지 않은 블로거 삭제 작업중...\n")
...
class second(QDialog):
    ...
    def main(self):
        ...
        if self.managing.isChecked():
            self.second_thread.manage = 1

        self.second_thread.t = t
