...
class secondThread(QThread):
    ...
    def __init__(self, ver, input_keyword, url_dirName, link_dirName, save_path, input_generate_post, api_key):
        super().__init__()
    ...
    def generate_keyword(self, ver, keyword, save_path, generate_post, api_key):
        ...
        while post_cnt < objec_cnt:
            ...
            if post_index == len(blog_list):
                ...
                blog_list = self.driver.find_elements(By.CSS_SELECTOR, '.title_link')[save_index:]
                save_index += len(blog_list)
                random.shuffle(blog_list)
            ...
            f = open(rf"{save_path}\[플뱅]_{today}_{keyword}_{current_time}.txt", 'w', encoding="UTF-8")
            ...
            f = open(rf"{save_path}\[플뱅]_{today}_{keyword}_{current_time}.txt", encoding="UTF-8")
            s = f.read()
            count = 0
            for i in s:
                if i == " " or i == "\n":
    ...
    def generate_blog_url(self, ver, path, save_path, generate_post, api_key):
        ...
        for url in blog_urls:
            ...
            while generate_cnt < generate_post:
                ...
                f = open(rf"{save_path}\[플뱅]_{today}_{date}_{last_url}.txt", 'w', encoding="UTF-8")
                ...
                f = open(rf"{save_path}\[플뱅]_{today}_{date}_{last_url}.txt", encoding="UTF-8")
                s = f.read()
                count = 0
                for i in s:
                    if i == " " or i == "\n":
                ...
                self.update_textbrowser.emit(f"{today}_{date}_{last_url} : {count}자")
    ...
    def generate_blog_link(self, ver, path, save_path, api_key):
        ...
        for link in blog_links:
            ...
            f = open(rf"{save_path}\[플뱅]_{today}_{blog_id}_{end_url}.txt", 'w', encoding="UTF-8")
            ...
            f = open(rf"{save_path}\[플뱅]_{today}_{blog_id}_{end_url}.txt", encoding="UTF-8")
            s = f.read()
            count = 0
            for i in s:
                if i == " " or i == "\n":
...
class Maindialog(QDialog):
    ...
    def start(self):
        ...
        if self.check_keyword.isChecked():
            ...
            self.second_thread = secondThread(ver, input_keyword, None, None, save_path, input_generate_post, api_key)
        ...
        # 체크박스 블로그 주소로 선택되었을 경우
            ...
            self.second_thread = secondThread(ver, None, self.url_dirName, None, save_path, input_generate_post, api_key)
        ...
        # 체크박스 블로그 링크로 선택되었을 경우
            ...
            self.second_thread = secondThread(ver, None, None, self.link_dirName, save_path, None, api_key)
