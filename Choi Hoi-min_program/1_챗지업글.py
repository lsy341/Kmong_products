...
class Maindialog(QDialog):
    ...
    def generate_keyword(self, keyword, save_path, generate_post, api_key):
        ...
        driver.get(f"https://m.search.naver.com/search.naver?where=m_view&sm=mtb_jum&query={keyword}")

        cnt = 1

        # 원고 생성 개수만큼 슬라이싱
        blog_list = driver.find_elements(By.CSS_SELECTOR, ".total_tit")[:generate_post]
        ...
        for blog in blog_list:
            ...
            f = open(rf"{save_path}\{today}_{keyword}_{current_time}.txt", 'w', encoding="UTF-8")
            ...
            f = open(rf"{save_path}\{today}_{keyword}_{current_time}.txt", encoding="UTF-8")
            s = f.read()
            count = 0
            for i in s:
                if i == " " or i == "\n":
    ...
    def generate_blog_url(self, path, save_path, generate_post, api_key):
        ...
        for url in blog_urls:
            ...
            # 블로그 정렬
            try:
                driver.find_element(By.CSS_SELECTOR, "#postlist_block > div.post_block__Q6T_o > div > div > button:nth-child(2)").click()
            except:
                pass


            # 블로그 포스팅 리스트 불러오기
            blog_list = driver.find_elements(By.CSS_SELECTOR, ".link__OVpnJ")[:generate_post]
            blog_list = list(enumerate(blog_list))
            random.shuffle(blog_list)

            # 블로그 작성 일자
            blog_date = driver.find_elements(By.CSS_SELECTOR, ".time__BwQ8u")
            ...
            for blog in blog_list:
                ...
                # 등록일자 변수
                date = blog_date[blog[0]].text.replace(". ", "_")

                # 블로그 포스팅 접속
                # 블로그 새 창으로 접속 후 본문 수집
                blog[1].send_keys(Keys.CONTROL, "\n")
                time.sleep(0.5)
                handles = driver.window_handles
                driver.switch_to.window(handles[1])
                ...
                f = open(rf"{save_path}\{today}_{date}_{last_url}.txt", 'w', encoding="UTF-8")
                ...
                f = open(rf"{save_path}\{today}_{date}_{last_url}.txt", encoding="UTF-8")
                s = f.read()
                count = 0
                for i in s:
                    if i == " " or i == "\n":
    ...
    def generate_blog_link(self, path, save_path, api_key):
        ...
        for link in blog_links:
            ...
            f = open(rf"{save_path}\{today}_{blog_id}_{end_url}.txt", 'w', encoding="UTF-8")
            ...
            f = open(rf"{save_path}\{today}_{blog_id}_{end_url}.txt", encoding="UTF-8")
            s = f.read()
            count = 0
            for i in s:
                if i == " " or i == "\n":
